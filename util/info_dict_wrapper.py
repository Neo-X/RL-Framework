...
class InfoDictStateWrapper(gym.Env):
  ...
  def step(self, action):
    ...
    """
    info_ = {}
    info_["agent_obs_before"] = self.getVisualState()
    info_["expert_obs_before"] = self.getImitationVisualState()
...
#     info["agent_pos"] = observation
    info["agent_obs"] = self.getVisualState()
#     info["expert_pos"] = self._task.build_target_obs()
    info["expert_obs"] = self.getImitationVisualState()
    info.update(info_)
...
class InfoDictStateWrapperLaikago(InfoDictStateWrapper):
  """An env wrapper that flattens the observation dictionary to an array."""
