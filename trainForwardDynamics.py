...
def trainForwardDynamics(settings):
    ...
    for round_ in range(rounds):
        ...
        if (round_ % settings['plotting_update_freq_num_rounds']) == 0:
            if (("train_LSTM_FD" in settings)
                and (settings["train_LSTM_FD"] == True)):
                state_, action_, resultState_, reward_, fall_, G_ts_, exp_actions, advantage_ = experience.get_multitask_trajectory_batch(batch_size=min(experience.samplesTrajectory(),lstm_batch_size_fd**2))
