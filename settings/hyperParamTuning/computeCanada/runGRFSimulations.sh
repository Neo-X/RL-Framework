...
do
	...
	# echo "$simConfigFile"
	command="SINGULARITYENV_TERRAINRL_PATH=/opt/TerrainRL SINGULARITYENV_RLSIMENV_PATH=/opt/RLSimulationEnvironments sbatch --time=50:00:00 --mem=32768M --cpus-per-task=24 ./settings/hyperParamTuning/computeCanada/test_run.sh 'singularity exec --cleanenv --home /home/gberseth/playground/RL-Framework/:/opt/RL-Framework /scratch/gberseth/playground/singularity/ubuntu_learning.simg python3.6 trainModel.py --config=settings/terrainRLImitate/CACLA/Flat_GRF_Tensorflow_NoPhase_30FPS.json -p 8 --rollouts=16 --on_policy=true --save_experience_memory=continual --num_rounds="$rounds" --continue_training=false --saving_update_freq_num_rounds=1 --plot=false --meta_sim_samples=3 --meta_sim_threads=3 --plotting_update_freq_num_rounds=5 --metaConfig="$metaConfig" --email_log_data_periodically=true --shouldRender=false'"
	echo $command
	eval $command
done
