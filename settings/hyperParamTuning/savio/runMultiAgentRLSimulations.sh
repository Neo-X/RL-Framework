...
# SINGULARITYENV_TERRAINRL_PATH=/opt/TerrainRL SINGULARITYENV_RLSIMENV_PATH=/opt/RLSimulationEnvironments singularity exec --cleanenv --home ~/playground/RL-Framework:/opt/RL-Framework /global/scratch/gberseth/SingularityBuilding/ubuntu_learning.img python3.6 tuneHyperParameters.py --config=settings/navgame2D/MADDPG/HRL_Tensorflow_NoViz-v3.json -p 2 --num_rounds=10 --continue_training=false --saving_update_freq_num_rounds=1 --plot=false --meta_sim_samples=3 --meta_sim_threads=3 --tuning_threads=2 --plotting_update_freq_num_rounds=5 --metaConfig="$metaExp" --email_log_data_periodically=true --shouldRender=false --bootstrap_samples=0 "$opts"'"
