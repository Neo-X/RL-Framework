...
for i in `seq 1 2`;
	...
	eai job submit --preemptable\
	    ...
	    --gpu 1 --cpu 5 --mem 16 \
	    -- bash -c "pushd /home/gberseth/playground/RL-Framework/; python3 trainModel.py --config=settings/terrainRLImitate/PPO/Imitation_Learning_GRF_StanQuad_LSTM_Critic_And_FD_Reward_Dual_Encode_Decode_VAE_2State_2_Advisarial_BCE_refresh.json -p 5 --shouldRender=false --log_comet=true --on_policy=fast --print_level=train --random_seed=$i"
	    # -- bash -c "pushd /home/gberseth/playground/RL-Framework/; python3 trainModel.py --config=settings/terrainRLImitate/PPO/Imitation_Learning_GRF_UniTree_1Sub_LSTM_Critic_And_FD_Reward_Dual_Encode_Decode_VAE_2State_2_Advisarial_BCE_refresh3.json -p 5 --shouldRender=false --log_comet=true --on_policy=fast --print_level=train --random_seed=$i"
