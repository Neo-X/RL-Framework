...
for i in `seq 1 3`;
	...
	eai job submit --preemptable\
	    ...
	    --gpu 0 --cpu 5 --mem 16 \
	    -- bash -c "pushd /home/gberseth/playground/RL-Framework/; python3 trainModel.py --config=settings/terrainRLImitate/TD3/Imitation_Learning_GRF_Dog.json -p 5 --shouldRender=false --log_comet=true --on_policy=fast --print_level=hyper_train --random_seed=$i"
	    # -- bash -c "pushd /home/gberseth/playground/RL-Framework/; python3 trainModel.py --config=settings/terrainRLImitate/PPO/Imitation_Learning_GRF_UniTree_1Sub_LSTM_FD_Reward_Dual_Encode_Decode_VAE_2State_2_Advisarial_BCE_refresh2.json -p 8 --shouldRender=false --log_comet=true --on_policy=fast --print_level=train --random_seed=$i"
