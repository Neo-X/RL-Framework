...
do
	...
	do
		...
		output=' | tee -a $BORGY_JOB_ID.out'
	 	arg="pushd /home/glen/playground/RL-Framework; python3 tuneHyperParameters.py --config=${simConfigFile} --metaConfig=${metaConfig} --meta_sim_samples=3 --meta_sim_threads=3 --tuning_threads=2 --num_rounds=${rounds} --plot=false --on_policy=fast --save_experience_memory=continual --continue_training=last --saving_update_freq_num_rounds=1 -p 4 --rollouts=12 --simulation_timeout=1200 --email_log_data_periodically=true --Multi_GPU=true --gpus=2 ${opts}"
	### GPU training
