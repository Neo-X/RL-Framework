...
def trainForwardDynamics(settings):
    ...
    _states, _actions, _result_states, _rewards, _falls, _G_ts, exp_actions__, _advantage = experience.get_batch(experience.samples())
    # states, actions, result_states, rewards, falls, G_ts, exp_actions, advantage

    ### Do some analysis
    for s in experience._state_history:
        sm = np.mean(s)
        if (sm < 0.5):
            print (sm)
    # print ("States: ", _states)
    states_avg = np.mean(_result_states, axis=0)
    ...
    fig1.savefig("viz_data_mean_state.svg")
    fig1.savefig("viz_data_mean_state.png")
