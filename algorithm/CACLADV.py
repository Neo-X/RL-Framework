...
class CACLADV(CACLA):
    ...
    def predict(self, state):
        ...
        if self._q_val()[0][0]> self._q_valTarget()[0][0]:
            action = self._q_action()[0]
        else:
            action = self._q_actionTarget()[0]
    ...
    def q_value(self, state):
        ...
        if self._q_val()[0][0]> self._q_valTarget()[0][0]:
            ...
            out = self._q_valTarget()
        out = scale_reward(out, self.getRewardBounds())[0]
        return out
       
    def bellman_errorTarget(self, states, actions, rewards, result_states):
        self.setData(states, actions, rewards, result_states)
