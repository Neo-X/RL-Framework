...
class ForwardDynamicsEncodingKerasEnsamble(KERASAlgorithm):
    
    def __init__(self, model, state_length, action_length, state_bounds, action_bounds, settings_, reward_bounds=0, print_info=False):

        super(ForwardDynamicsEncodingKerasEnsamble,self).__init__(model, state_length, action_length, state_bounds, action_bounds, reward_bounds, settings_)
        ...
        model_._encoder = Model(inputs=[model_.getStateSymbolicVariable()], outputs=model_._networkActMiddle)
        
        # model_._decoder = Model(inputs=[model_._EncodedStateAct], outputs=model_._actor)
         
        # model_._forward_dynamics_net = model_._decoder(model_._encoder(inputs_))
        ...
        ForwardDynamicsEncodingKerasEnsamble.compile(self)
    ...
    def getEncoding(self, state, member=0):
        # print("State: ", state)
        # print("Action: ", action)
        state = np.array(norm_state(state, self._state_bounds), dtype=self.getSettings()['float_type'])
        # action = np.array(norm_action(action, self._action_bounds), dtype=self.getSettings()['float_type'])
        state_ = scale_state(np.array(self._models[member]._encoder.predict([state])), self._state_bounds)
        # print("state:", state_)
        return state_
