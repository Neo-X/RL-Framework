...
class RND(KERASAlgorithm):
    
    def __init__(self, model, state_length, action_length, state_bounds, action_bounds, settings_, reward_bounds=0, print_info=False):

        super(RND,self).__init__(model, state_length, action_length, state_bounds, action_bounds, reward_bounds, settings_)
        self._model = model
        self._modelTarget = type(self._model)(state_length, state_length, state_bounds, action_bounds, reward_bounds, settings_, print_info=print_info)
        ...
        inputs_ = [self._modelTarget.getStateSymbolicVariable()] 
        self._modelTarget._forward_dynamics_net = Model(inputs=inputs_, outputs=self._modelTarget._actor)
        if (print_info):
            print("FD Net summary: ", self._modelTarget._forward_dynamics_net.summary())
        self._modelTarget._reward_net = Model(inputs=inputs_, outputs=self._modelTarget._reward_net)
        if (print_info):
            print("Reward Net summary: ", self._modelTarget._reward_net.summary())
        
        self._forward = self._model.getForwardDynamicsNetwork()([self._model.getStateSymbolicVariable()])
        self._reward = self._model.getRewardNetwork()([self._model.getStateSymbolicVariable()])
    ...
    def compile(self):
        ...
        self.reward = K.function([self._model.getStateSymbolicVariable(), K.learning_phase()], [self._reward])
    ...
    def updateTargetModel(self):
        """
            Never update random target network
        """
        pass
    ...
    def train(self, states, actions, result_states, rewards, updates=1, batch_size=None, lstm=False, datas=[], trainInfo=None):
        ...
        if ( self._train_combined_loss ):
            ...
            rands_pred = self._modelTarget.getForwardDynamicsNetwork().predict([states])
            score = self._model.getForwardDynamicsNetwork().fit([states], rands_pred,
            ...
            if ( self.getSettings()['train_reward_predictor']):
                ...
                score = self._model.getRewardNetwork().fit([states], rewards,
                  epochs=updates, batch_size=batch_size_,
                  verbose=0,
                  shuffle=True
                  # callbacks=[early_stopping],
    ...
    def predict(self, state, action):
        ...
        pred = self.fd([state,0])[0]
        pred = pred[:,:self._state_length] ### Cut off std
        # print ("pred: ", pred[:,:self._action_length])
        state_ = scale_state(pred, self._state_bounds)
        return state_
    
    def predict_intrinsic_reward(self, state, action):
        state = np.array(norm_state(state, self._state_bounds), dtype=self.getSettings()['float_type'])
#         action = np.array(norm_action(action, self._action_bounds), dtype=self.getSettings()['float_type'])
        pred_rand = self._modelTarget.getForwardDynamicsNetwork().predict([state])
        pred = self.fd([state,0])[0]
        r_ = r = np.sum(np.square(pred_rand - pred))
        return r_
    ...
    def predict_reward(self, state, action):
        ...
        state = np.array(norm_state(state, self._state_bounds), dtype=self.getSettings()['float_type'])
        action = np.array(norm_action(action, self._action_bounds), dtype=self.getSettings()['float_type'])
        predicted_reward = self.reward([state, 0])[0]
