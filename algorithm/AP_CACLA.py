...
from model.LearningUtil import *
...
class AP_CACLA(AlgorithmInterface):
    
    def __init__(self, model, n_in, n_out, state_bounds, action_bounds, reward_bound, settings_):

        super(AP_CACLA,self).__init__(model, n_in, n_out, state_bounds, action_bounds, reward_bound, settings_)
        ...
        if (self.getSettings()['use_previous_value_regularization']):
            ...
            self._kl_firstfixed = T.mean(kl(self._q_valsActA, T.ones_like(self._q_valsActA) * self.getSettings()['exploration_rate'], self._q_valsActTarget, T.ones_like(self._q_valsActTarget) * self.getSettings()['exploration_rate'], self._action_length))
    ...
    def compile(self):
        ...
        self._get_actor_regularization = theano.function([], [self._actor_regularization],
                                                         givens={self._model.getStateSymbolicVariable(): self._model.getStates(),
                                                                    self._KL_Weight: self._kl_weight_shared})
        self._get_actor_loss = theano.function([], [self._actLoss], givens={
            self._model.getStateSymbolicVariable(): self._model.getStates(),
            # self._model.getResultStateSymbolicVariable(): self._model.getResultStates(),
            # self._model.getRewardSymbolicVariable(): self._model.getRewards(),
            self._model.getActionSymbolicVariable(): self._model.getActions(),
